...
/**
 * AiDualStrategy — hedge-mode, grid-driven dual strategy.
 *
 * Phase 1 shell: a byte-copy of the AI Reversal strategy retagged to AI_DUAL and
 * flipped to HEDGE (dual-side) position mode. The reversal machinery below is
 * preserved verbatim for Phase 2 (TREND). In RANGE, the grid is built from the
 * volume profile on the first tick (initializeGrid, Task 5/8) — NOT from an AI
 * PLAN — so the reversal cycle-start consult in start() is intentionally
 * neutralized. The original reversal doc block follows for reference:
 *
 ...
 */
class AiDualStrategy extends TradingBase {
  ...
  async start(config = {}) {
    ...
    if (!this.currentInitialSize || this.currentInitialSize <= 0) {
      throw new Error('AiDualStrategy.start: invalid initialSize');
    }
    ...
    // Grid-driven start (dual/RANGE): NO AI PLAN consult is fired at cycle
    // start. Leave executionState IDLE (no consult in flight) and gridLines
    // empty so the first tick builds the RANGE grid from the volume profile
    // via initializeGrid (Task 5/8) instead of from an AI PLAN.
    ...
    await this.addLog('AiDualStrategy running — awaiting first tick to build grid.');
    await this.saveState();

    // ---- Reversal PLAN cycle-start consult: INTENTIONALLY DISABLED for dual ----
    // The reversal strategy fired a single `_requestPlan('cycle_start')` here to
    // have the AI set bull/bear levels. The dual strategy in RANGE builds its
    // grid from the volume profile on the FIRST TICK (initializeGrid, Task 5/8),
    // so no AI PLAN consult is issued at start. The planner/executor/riskGuard/
    // marketContext remain instantiated above for Phase 2 TREND. Original kickoff:
  ...
  async resume(snapshot) {
    if (!snapshot) throw new Error('AiDualStrategy.resume: missing snapshot');
...
export { AiDualStrategy };
export default AiDualStrategy;
