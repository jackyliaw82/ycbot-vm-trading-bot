...
test('requireVmOwner: the VM owner is allowed through', () => {
  ...
  const { res, nextCalled } = run(mw, { uid: 'abc123', method: 'POST', path: '/reversal-ladder/start' });
...
test('requireVmOwner: a different user is refused with NOT_VM_OWNER', () => {
  const mw = createRequireVmOwner(() => 'abc123');
  const { res, nextCalled } = run(mw, { uid: 'someone-else', method: 'POST', path: '/reversal-ladder/start' });
...
test('requireVmOwner: an admin uid passes through to any VM (fleet release rollout)', () => {
  const mw = createRequireVmOwner(() => 'abc123');
  const { nextCalled } = run(mw, { uid: 'admin-uid-1', method: 'POST', path: '/reversal-ladder/status' });
...
test('requireVmOwner: an admin still passes when the owner is unresolved (metadata outage must not lock admins out)', () => {
  const mw = createRequireVmOwner(() => null);
  const { res, nextCalled } = run(mw, { uid: 'admin-uid-1', method: 'POST', path: '/reversal-ladder/status' });
...
test('requireVmOwner: a missing verified uid is 401, not a silent pass', () => {
  const mw = createRequireVmOwner(() => 'abc123');
  const { res, nextCalled } = run(mw, { method: 'POST', path: '/reversal-ladder/start' });
...
test('requireVmOwner: getOwnerUid is called per request, not captured once', () => {
  ...
  const first = run(mw, { uid: 'abc123', method: 'GET', path: '/reversal-ladder/status' });
  ...
  const second = run(mw, { uid: 'abc123', method: 'GET', path: '/reversal-ladder/status' });
