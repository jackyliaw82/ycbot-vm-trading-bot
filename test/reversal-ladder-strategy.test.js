...
// Before the first tick has anchored the ladder, there is no anchor to derive
// a level from. Printing a bogus number (or "NaN") would be worse than saying
// it is pending.
...
test('turning trailing on is accepted even with no ladder yet or in TREND (it just lies dormant)', async () => {
  const noLadder = ladderStrategy();
  noLadder.ladderLines = [];
  assert.deepEqual(await noLadder.setTrailDirection('UP'), { trailDirection: 'UP' });
...
test('trailing does not fire while there is no ladder yet', async () => {
  const s = ladderStrategy({ anchor: 100 });
  s.ladderLines = [];
  s.trailDirection = 'DOWN';
  let fired = false;
