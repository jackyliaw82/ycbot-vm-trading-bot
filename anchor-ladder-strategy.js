...
class AnchorLadderStrategy extends TradingBase {
  ...
  async _closeConsolidated(reason) {
    ...
    // the same direction — crossing the anchor flattens before the other side
    // can open (ladder-crossings rule 1).
    ...
    if (!verified) {
      try { await this._refreshCurrentPosition(); } catch (_) { /* flag is set by the refresh */ }
    ...
    if (!verified) {
      await this.addLog(
        `WARNING: close (${reason}) could NOT be verified — no WS fill event, no REST ack, and Binance ` +
        `has not verified the position as closed. Position state left INTACT. Verify manually on Binance.`,
      );
      return false;
    }
  ...
  async _flattenAtAnchor() {
    ...
    if (!hasOpenLegs && !(this.activePosition && this.activePosition.quantity > 0)) return;
